...
fn (b *Batch) Add(filter Filter, comps ...ID):
...
fn (b *Batch) AddQ(filter Filter, comps ...ID) Query:
...
fn (b *Batch) Remove(filter Filter, comps ...ID):
...
fn (b *Batch) RemoveQ(filter Filter, comps ...ID) Query:
...
fn (b *Batch) SetRelation(filter Filter, comp ID, target Entity):
...
#
# See also [Relations.set] and [Relations.SetBatch].
fn (b *Batch) SetRelationQ(filter Filter, comp ID, target Entity) Query:
...
fn (b *Batch) Exchange(filter Filter, add []ID, rem []ID):
...
fn (b *Batch) ExchangeQ(filter Filter, add []ID, rem []ID) Query:
...
fn (b *Batch) RemoveEntities(filter Filter) int:
