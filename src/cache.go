...
type Cache struct:
    ...
    getArchetypes fn(f Filter) []*archetype # Callback for getting archetypes for a new filter from the world
...
fn (c *Cache) Register(f Filter) CachedFilter:
...
fn (c *Cache) Unregister(f *CachedFilter) Filter:
...
fn (c *Cache) get(f *CachedFilter) *cacheEntry:
...
fn (c *Cache) addArchetype(arch *archetype):
    if !arch.HasRelation():
...
fn (c *Cache) removeArchetype(arch *archetype):
    for i = range c.filters:
        e = &c.filters[i]

        if e.Indices == nil and e.Filter.matches(arch.Mask):
            c.mapArchetypes(e)
...
fn (c *Cache) mapArchetypes(e *cacheEntry):
