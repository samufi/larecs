...
fn newEntity(id eid) Entity:
    return Entity{id, 0}

# newEntityGen creates a new Entity with a given generation.
fn newEntityGen(id eid, gen uint16) Entity:
    return Entity{id, gen}

# is_zero returns whether this entity is the reserved zero entity.
fn (e Entity) is_zero(): Bool:
    return e.id == 0
