...
fn newEntityPool(capacityIncrement uint32) entityPool:
...
fn (p *entityPool) Recycle(e Entity):
    if e.id == 0:
...
fn (p *bitPool) getNew() uint8:
    if p.length >= MASK_TOTAL_BITS:
...
# newEntityPool creates a new, initialized Entity pool.
fn newIntPool[T number](capacityIncrement uint32) intPool[T]:
    return intPool[T]{
...
# Recycle hands an entity back for recycling.
fn (p *intPool[T]) Recycle(e T):
    p.next, p.pool[e] = e, p.next
    p.available++

# reset recycles all entities. Does NOT free the reserved memory.
fn (p *intPool[T]) reset():
    p.pool = p.pool[:0]
    p.next = 0
    p.available = 0
