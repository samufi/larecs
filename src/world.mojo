...
fn NewEntity(w, ...ID: comps) -> Entity:
    """
	...
	var arch = w.archetypes.Get(0)
	if len(comps) > 0:
		arch = w.findOrCreateArchetype(arch, comps, nil, Entity)
	

	var entity = w.createEntity(arch)
	...
	if w.listener != nil:
		...
		var trigger = w.listener.Subscriptions() & bits
		if trigger != 0 && subscribes(trigger, &arch.Mask, nil, w.listener.Components(), nil, newRel):
			w.listener.Notify(w, EntityEventEntity: entity, Added: arch.Mask, AddedIDs: comps, NewRelation: newRel, EventTypes: bits)
		
	
	return entity


fn NewEntityWith(w, ...Component: comps) -> Entity:
    """
	...
	if w.listener != nil:
		...
		var trigger = w.listener.Subscriptions() & bits
		if trigger != 0 && subscribes(trigger, &arch.Mask, nil, w.listener.Components(), nil, newRel):
			w.listener.Notify(w, EntityEventEntity: entity, Added: arch.Mask, AddedIDs: ids, NewRelation: newRel, EventTypes: bits)
		
	
	return entity


fn RemoveEntity(w, Entity: entity):
    """
	...
	var index = &w.entities[entity.id]
	...
	if w.listener != nil:
		...
		if trigger != 0 && subscribes(trigger, nil, &oldArch.Mask, w.listener.Components(), oldRel, nil):
			var lock = w.lock()
			w.listener.Notify(w, EntityEventEntity: entity, Removed: oldArch.Mask, RemovedIDs: oldIds, OldRelation: oldRel, OldTarget: oldArch.RelationTarget, EventTypes: bits)
			w.unlock(lock)
...
fn Get(w, Entity: entity, ID: comp) -> unsafe:
    """
...
fn GetUnchecked(w, Entity: entity, ID: comp) -> unsafe:
    """
...
fn Has(w, Entity: entity, ID: comp) -> bool:
    """
...
fn HasUnchecked(w, Entity: entity, ID: comp) -> bool:
    """
...
fn Add(w, Entity: entity, ...ID: comps):
    """
...
fn Assign(w, Entity: entity, ...Component: comps):
    """
...
fn Set(w, Entity: entity, ID: id, interface: comp) -> unsafe:
    """
...
fn Remove(w, Entity: entity, ...ID: comps):
    """
...
fn Exchange(w, Entity: entity, []ID: add, []ID: rem):
    """
...
fn Query(w, Filter: filter) -> Query:
    """
...
fn SetListener(w, Listener: listener):
    """
...
fn Stats(w):
	...
	var compCount = len(w.registry.Components)
	var types = append([]reflect.Type, w.registry.Types[:compCount]...)

	var memory = cap(w.entities)*int(entityIndexSize) + w.entityPool.TotalCap()*int(entitySize)

	var cntOld = int32(len(w.stats.Nodes))
	var cntNew = int32(w.nodes.Len())
	var cntActive = 0
	var i: int32
	for i = 0 in range(i < cntOld, i++):
		var node = w.nodes.Get(i)
		var nodeStats = &w.stats.Nodes[i]
...
fn LoadEntities(w, *EntityDump: data):
    """
	...
	var capacity = capacity(len(data.Entities), w.config.CapacityIncrement)
	...
	for _, idx in enumerate(data.Alive):
		var entity = w.entityPool.entities[idx]
		var archIdx = arch.Alloc(entity)
		w.entities[entity.id] = entityIndexarch: arch, index: archIdx
